...
> Evidence-bound recovery can satisfy the Completion Contract when every ask,
> evidence binding, citation and quality check passes; it remains marked as
> degraded synthesis and does not count as primary-writer performance. If
> retrieval produced no admitted claim, safe evidence-backed limitation may
> still be delivered as `partial`; no recoverable evidence remains `failed`.
> Persisted pre-lineage Briefs are diagnostic-only compatibility inputs and
> cannot bypass Claim Admission.
>
...
A worker lease is an admission record, not a run-token reservation. Its `token_ceiling` is the worker lifetime limit; run-level reserved tokens include only non-worker in-flight requests and worker in-flight LLM requests. Creating two 80K medium workers therefore reserves zero tokens until each model call is actually authorized. Committing an actual usage smaller than its estimate refunds the difference atomically. For finite worker leases, retrieval-capable calls can consume at most 85% of the worker token ceiling; the remaining 15% is available only in the `finalize` phase. This ensures a worker stopped by its local token cap can still attempt one bounded structured finalization. The Run token ceiling remains authoritative and is never exceeded.
...
Finalization is armed before an LLM request when projected usage would risk the token ceiling, when final LLM calls must be preserved, or when the worker reaches a soft wall deadline. The token threshold is adaptive between 40% and 80%. The wall deadline reserves 20–45 seconds, giving a 150-second worker its first 120 seconds for research and a 300-second worker its first 255 seconds. The same reserve is enforced as an inner timeout around the retrieval-capable stream. The outer step deadline adds one bounded finalizer window of at most 90 seconds, capped by the remaining run deadline, so a slow provider response cannot cancel structured finalization as soon as the research window ends. With zero retrieval and zero evidence, the first retrieval is still authorized after the soft deadline unless a hard deadline, hard budget, or cancellation has fired. Once evidence exists, the current request receives a Finalization Mode instruction, retrieval is disabled, and remaining capacity is preserved for the structured WorkerResult. Hard token, call, and timeout caps remain runaway-protection ceilings. If finalization also reaches the hard deadline or budget, artifact-backed evidence is salvaged and the exact failure reason is preserved.
...
At ingestion, a task with blank question/ask IDs may inherit lineage only from one exact match between its `target_gaps` and a Brief research question. Ambiguous or approximate matches remain unbound and are rejected by Claim Admission; the runtime does not use semantic guessing to manufacture question coverage.

Worker lifecycle is classified after finalization from structured validity, accepted findings, admitted evidence, and the terminal reason. A local tool failure is not a worker failure: evidence plus accepted findings and a normal stop is complete; evidence with an abnormal stop is stopped/partial and remains ingestible; only zero evidence and zero accepted findings can become a terminal worker failure. Coverage must not promote an evidence-bearing partial Worker to a global `worker_failed` gap. `last_tool_error`, `fail_reason`, and `stop_reason` are separate diagnostics.
...
If synthesis tokens are low or the provider fails while usable evidence exists, the runtime first compiles an evidence-bound answer from admitted claims. A complete, cited answer may pass as `success` with `synthesis_degraded=true`; the system does not claim primary synthesis succeeded. If a required question lacks an admitted claim, it renders a deterministic user-readable partial result. Partial content:
