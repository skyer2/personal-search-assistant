...
`app/research/delivery/answer_contract.py` defines the Answerability and Answer Completeness gates plus the `QuestionAnswer` / `FinalAnswer` contract. Every key question receives a direct answer, finding references, evidence references, confidence, and limitations. Trend and future-oriented questions are marked as evidence-backed `inference` or `forecast`; deterministic recovery presents direct answers first, then reasoning, citations, and limitations. Quality evaluates the final delivery, so partial workers may still yield success, while a complete recovery after synthesis failure is explicitly `degraded_success`.
