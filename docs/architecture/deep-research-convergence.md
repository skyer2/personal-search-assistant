...
The normal pack is capped at 8K input tokens and the compact retry at 4K, with a 30K hard maximum. A synthesis timeout retries once with the strictly smaller compact pack; the retry input cannot be identical to the first attempt. Each attempt records its selected-pack size, full prompt size, digest size, model/provider, time to first token when available, wall time, actual token usage, finish reason, and failure reason. A compact retry or deterministic Answer Compiler recovery sets `synthesis_degraded=true` and records diagnostics, while a complete grounded report remains the canonical `success` outcome. The evidence dump is reserved for unanswerable questions or missing usable evidence. The normal primary path targets under 120 seconds P95; Deep Debug permits under 180 seconds during integration.
...
Evidence Pack selection and digest resolution are separate contracts. The pack selects canonical evidence IDs; `SynthesisContextBuilder` resolves each ID through its `EvidenceRecord.artifact_ref` and citation aliases and reads the real artifact summary/content as the excerpt. Synthesis is not invoked when selected findings, evidence references, or non-empty digests are missing; the runtime records `synthesis_evidence_digest_missing` and only falls back to an evidence-oriented partial result when Answerability is false.

### Answer Contract and delivery gates

`app/research/delivery/answer_contract.py` defines the Answerability and Answer Completeness gates plus the `QuestionAnswer` / `FinalAnswer` contract. `app/research/domain/completion.py` is the sole terminal authority. Every key question receives a direct answer, finding references, evidence references, confidence, and limitations. Trend and future-oriented questions are marked as evidence-backed `inference` or `forecast`; deterministic recovery presents direct answers first, then reasoning, citations, and limitations. Quality evaluates the final delivery, so partial workers may still yield success when the Completion Contract is satisfied; synthesis degradation remains a diagnostic field.
