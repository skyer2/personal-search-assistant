...
> `persist_loop_state` 默认 false。产品入口见 [PERSONAL_SEARCH.md](./PERSONAL_SEARCH.md)。
...
API / UI
   ...
   → Research Runtime (StateGraph + SQLite checkpointer)
   → WorkerRuntime (LangChain now; DeepSeek later)
   → Web / Fetch / Files  +  Artifact / Evidence Store
```
...
ANSWER / SEARCH 在 Task Router 处分流，不进上述节点。
...
|--|--------------------------------------------------------------------------|--------|
| 入口 | 每问都进主 Agent | ANSWER / SEARCH / RESEARCH |
...
面试运维面：`GET /api/harness/capabilities` 返回当前 `graph_runtime_enabled`、三档 `search_modes`、个人默认来源。
