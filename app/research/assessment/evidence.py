"""Evidence assessment aligned with Brief criterion support."""
...
def assess_evidence(state: dict[str, Any]) -> EvidenceAssessment:
    ...
    supported = [
        ...
        if any(str(item) in evidence_ids for item in claim.get("evidence_ids") or [])
    ]
    ...
    independent = len(
        {str(row.get("source_id") or "") for row in records if row.get("source_id")}
    )
    conflicts = [str(item) for item in judgement.get("conflicts") or []]
    criteria = [
        row for row in judgement.get("criteria") or [] if isinstance(row, dict)
    ]
    all_supported = bool(criteria) and all(
        str(row.get("status") or "") == "supported" for row in criteria
    )
    if records and all_supported and not conflicts:
        status = EvidenceStatus.SUFFICIENT
        reasons = ["criterion_evidence_sufficient"]
    elif records:
        status = EvidenceStatus.PARTIAL
        reasons = ["criterion_evidence_partial"]
    else:
